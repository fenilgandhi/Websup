...
class YowsupWebLayer(YowInterfaceLayer):
	...
	def send_message(self, object):
		mobilenumber = object.to_contact.number
		text_message = object.message_format.msg_text

		if self.assertConnected():
			if sys.version_info >= (3, 0):
				text_message = text_message.encode("utf-8")

			if bool(object.message_format.msg_img1) and bool(os.path.isfile(object.message_format.msg_img1.path)):
				image = object.message_format.msg_img1.path
				result = self.media_send(mobilenumber, image, RequestUploadIqProtocolEntity.MEDIA_TYPE_IMAGE)
			self.message_send(mobilenumber, text_message)
